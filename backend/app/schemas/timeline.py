...
class TimelineEventResponse(BaseSchema):
    ...
    event_type: str = Field(
        serialization_alias="type", description="Event type: version, proposal, edit, upload, etc."
    )
    ...
    actor: str | None = Field(
        default=None, serialization_alias="user", description="User who performed the action"
    )

    # Map event_metadata → metadata (frontend expects "metadata")
    event_metadata: dict[str, Any] | None = Field(
        default=None, serialization_alias="metadata", description="Additional event data"
    )

    # Map created_at → timestamp (frontend expects "timestamp")
    created_at: datetime = Field(
        serialization_alias="timestamp", description="When the event occurred"
    )
