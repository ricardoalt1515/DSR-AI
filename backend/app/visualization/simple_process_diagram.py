...
class SimpleProcessDiagram:
    ...
    def _position_equipment(
        self, equipment_data: list[dict], canvas_width: float
        ...
        sorted_equipment = sorted(
            equipment_data, key=lambda eq: stage_order.get(eq.get("stage", "secondary"), 2)
        )
    ...
    def _draw_equipment(self, ax, equipment: dict, position: tuple[float, float], index: int):
        ...
        if complexity == "complex" or "reactor" in eq_type.lower():
            self._draw_reactor_vessel_premium(
                ax, x, y, fill_color, accent_color, eq_type, equipment_tier
            )
        elif "clarificador" in eq_type.lower() or "sediment" in eq_type.lower():
            self._draw_clarifier_premium(
                ax, x, y, fill_color, accent_color, eq_type, equipment_tier
            )
            ...
            self._draw_generic_equipment_premium(
                ax, x, y, fill_color, accent_color, eq_type, equipment_tier
            )
    ...
    def _draw_reactor_vessel_premium(
        self, ax, x: float, y: float, fill_color: str, accent_color: str, label: str, tier: str
    ):
    ...
    def _draw_clarifier_premium(
        ...
        center_weir = Circle(
            (x, y), 8, facecolor=accent_color, edgecolor=self.colors["text_dark"], linewidth=1
        )
    ...
    def _draw_pump_premium(
        ...
        impeller = Circle(
            (x, y), 12, facecolor=accent_color, edgecolor=self.colors["text_dark"], linewidth=1
        )
    ...
    def _draw_filter_premium(
        ...
        for i in range(4):
            y_line = y - 15 + i * 6
            ax.plot([x - 25, x + 25], [y_line, y_line], color=accent_color, linewidth=3, alpha=0.7)

        # Distribuidor superior premium
        distributor = Rectangle(
            (x - 25, y + 15),
            50,
            8,
            facecolor=accent_color,
    ...
    def _draw_generic_equipment_premium(
        self, ax, x: float, y: float, fill_color: str, accent_color: str, label: str, tier: str
    ):
        """Dibuja equipo genérico premium"""
        equipment = FancyBboxPatch(
            (x - 35, y - 20),
            70,
            40,
        ...
        for angle in range(0, 360, 45):
            ...
            ax.plot(
                [x + np.cos(rad) * 8, tooth_x],
                [y + np.sin(rad) * 8, tooth_y],
                color=self.colors["text_dark"],
                linewidth=2,
            )
    ...
    def _draw_connections(
        self, ax, positioned_equipment: list[tuple[dict, tuple[float, float]]], system_info: dict
    ):
    ...
    def _draw_process_line(
        self, ax, start_pos: tuple[float, float], end_pos: tuple[float, float], flow_rate: float
    ):
        ...
        if flow_rate > 0:
            ...
            ax.text(mid_x, mid_y + 18, f"{flow_rate:,.0f} m³/d", ha="center", va="bottom", **typo)

    def _draw_inlet_line(
        self, ax, start_pos: tuple[float, float], end_pos: tuple[float, float], flow_rate: float
    ):
        ...
        # Premium line
        ax.plot([x1, x2 - 40], [y1, y2], color=self.colors["process_green"], linewidth=4, alpha=0.9)
    ...
    def _draw_outlet_line(
        ...
        # Premium line
        ax.plot([x1 + 40, x2], [y1, y2], color=self.colors["treated_green"], linewidth=4, alpha=0.9)
    ...
    def _add_diagram_title(self, ax, system_info: dict, canvas_width: float):
        ...
        flow_rate = system_info.get("flow_rate_m3_day", 0)
        subtitle = (
            f"Caudal de diseño: {flow_rate:,.0f} m³/día"
            if flow_rate > 0
            else "Sistema de tratamiento adaptativo"
        )
    ...
    def _add_system_info(self, ax, system_info: dict, canvas_height: float):
        ...
        if eff_text:
            ...
            ax.text(
                50,
                info_y,
                f"Eficiencias de tratamiento: {eff_text}",
                ha="left",
                va="bottom",
                **info_typo,
            )
