...
try:
    from app.visualization.simple_process_diagram import PremiumVisualConfig, simple_process_diagram
...
class PremiumPlotlyConfig:
    ...
    COLOR_SCHEMES = {
        ...
        "capex_breakdown": [
            ...
            COLORS["process_green"],
            COLORS["warning_orange"],
        ],
        "opex_breakdown": [
            COLORS["critical_red"],
            COLORS["critical_red_light"],
            COLORS["warning_orange"],
            COLORS["premium_gray"],
        ],
        "cash_flow": {
...
class PremiumChartGenerator:
    ...
    def generate_executive_charts(self, metadata: dict[str, Any]) -> dict[str, str]:
        ...
        try:
            ...
            if isinstance(agent_data, dict):
                ...
                logger.info(
                    f"  - Duration: {agent_data.get('implementation_months', 'NOT AVAILABLE')} months"
                )
                logger.info(
                    f"  - CAPEX breakdown: {'YES' if agent_data.get('capex_breakdown') else 'NO'}"
                )
                logger.info(
                    f"  - OPEX breakdown: {'YES' if agent_data.get('opex_breakdown') else 'NO'}"
                )
                logger.info(
                    f"  - Efficiencies: {'YES' if agent_data.get('treatment_efficiency') else 'NO'}"
                )
                logger.info(f"  - ROI: {agent_data.get('roi_percent', 'NOT AVAILABLE')}%")
                logger.info(
                    f"  - Payback: {agent_data.get('payback_years', 'NOT AVAILABLE')} years"
                )
    ...
    def _enhance_equipment_with_process_info(
        self, equipment: dict, semantic_properties: dict
    ) -> dict:
        """
    ...
    def _sort_equipment_by_process_flow(self, equipment_list: list[dict]) -> list[dict]:
        ...
        logger.info(
            f"🔄 Equipos ordenados en secuencia de proceso: {[eq.get('type') for eq in sorted_equipment]}"
        )
    ...
    def _calculate_system_metrics(
        self, equipment_list: list[dict], efficiencies: dict[str, float]
    ) -> dict:
        """
        ...
        logger.info(
            f"📊 Métricas del sistema: Capacidad={total_capacity} m³/d, Potencia={total_power} kW, Equipos={equipment_count}"
        )
        return metrics
    ...
    def _build_adaptive_diagram_structure(self, process_groups: dict, system_metrics: dict) -> dict:
        ...
        for group_name in sorted(process_groups.keys(), key=lambda x: group_order.get(x, 5)):
            ...
            group_info["visual_emphasis"] = self._calculate_visual_emphasis(
                group_info, system_metrics
            )
    ...
    def _extract_technical_specs(self, equipment: dict) -> dict:
        ...
        return {
            ...
            "efficiency": equipment.get("efficiency"),
        }
    ...
    def _create_premium_equipment_node(self, equipment: dict, index: int, group_info: dict) -> dict:
        ...
        return {"symbol": symbol, "shape": shape, "label": sanitized_label}

    def _create_premium_equipment_style(
        self, equipment: dict, eq_id: str, node_design: dict
    ) -> dict:
        ...
        if "color" in equipment:
            ...
            return {"lines": style_lines, "class_name": class_name}
        else:
            ...
            return {"lines": [], "class_name": style_mapping.get(stage, "equipmentStyle")}
    ...
    def _create_intelligent_connection(
        ...
        return {"label": label, "type": connection_type}
    ...
    def _generate_premium_pid_styles(self, colors: dict, system_metrics: dict) -> list[str]:
        ...
        if system_metrics.get("process_complexity") == "complex":
            styles.append(
                f"    classDef complexSystem fill:{colors['tech_purple']},stroke:{colors['warning_orange']},stroke-width:3px,color:{colors['clean_white']},font-size:12px,font-weight:bold"
            )
    ...
    def _create_premium_inlet_outlet_nodes(
        self, system_metrics: dict, avg_efficiency: float
    ) -> dict[str, list[str]]:
        """
    ...
    def _create_premium_efficiency_panel(
        self, efficiencies: dict[str, float], system_metrics: dict
    ) -> list[str]:
        """
    ...
    def _create_premium_tech_specs_panel(
        self, system_metrics: dict, equipment_list: list[dict]
    ) -> list[str]:
        """
    ...
    def _create_final_process_connection(
        self, previous_node: str, system_metrics: dict
    ) -> list[str]:
        """
        ...
        connection_label = (
            f"🌊 EFLUENTE FINAL - ✅ {capacity:,.0f} m³/día TRATADOS - 🎯 LISTO PARA DESCARGA"
        )

        return [f'    {previous_node} -->|"{connection_label}"| SALIDA', ""]
    ...
    def _get_equipment_css_class(self, eq_type: str) -> str:
        ...
        if any(word in eq_type for word in ["REJILLA", "CRIBADO", "DESARENADOR", "DESENGRASADOR"]):
            ...
            word in eq_type for word in ["REACTOR", "BIOLOGICO", "MBBR", "LODOS", "ACTIVADOS"]
        ):
            return "biologico"
        elif any(
            word in eq_type for word in ["COAGULACION", "FLOCULACION", "SEDIMENTACION", "DAF"]
        ):
            return "fisicoquimico"
        elif any(
            word in eq_type
            for word in ["FILTRO", "FILTRACION", "MEMBRANA", "ULTRAFILTRO", "NANOFILTRO"]
        ):
            return "filtracion"
    ...
    def _render_mermaid_to_base64(self, mermaid_content: str) -> str:
        ...
        try:
            ...
            cmd = [
                ...
                output_file,
                "-w",
                "2400",  # Ancho aumentado para mejor aprovechamiento del PDF
                "-H",
                "1800",  # Altura aumentada para diagramas verticales
                "--scale",
                "4",  # Escala muy alta para máxima legibilidad
                "--backgroundColor",
                "white",
                "--theme",
                "neutral",  # Tema válido (corregido de 'base' a 'neutral')
            ]
            ...
            if result.returncode != 0:
                ...
                logger.debug(
                    mermaid_content[:500] + "..." if len(mermaid_content) > 500 else mermaid_content
                )
    ...
    def _create_matplotlib_fallback(self, agent_data: dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.info(
                f"🧠 Equipos clasificados con análisis semántico: {len(classified_equipment)}"
            )
            ...
            ax.text(
                14,
                19.5,
                "PLANTA DE TRATAMIENTO DE AGUA RESIDUAL - DISEÑO P&ID PROFESIONAL",
                fontsize=22,
                ha="center",
                va="center",
            ...
            ax.text(
                ...
                ha="center",
                va="center",
                color=premium_colors["tech_purple"],
                fontweight="bold",
            )

            # Líneas decorativas premium para encuadre 4K
            ax.plot(
                [-1, 29], [18.2, 18.2], color=premium_colors["primary_blue"], linewidth=4, alpha=0.9
            )
            ax.plot(
                [-1, 29],
                [17.8, 17.8],
                color=premium_colors["process_green"],
                linewidth=2,
                alpha=0.7,
            )
            ...
            inlet_main = FancyBboxPatch(
                (1, 9.0),
                5.0,
                3.5,  # Mucho más grande para 4K
            ...
            for i, equipment in enumerate(classified_equipment):
                pos = (
                    equipment_positions[i]
                    if i < len(equipment_positions)
                    else equipment_positions[-1]
                )
                ...
                if i == 0:
                    ...
                    prev_pos = (
                        equipment_positions[i - 1]
                        if i - 1 < len(equipment_positions)
                        else equipment_positions[-1]
                    )
                    prev_x = previous_x
                    self._draw_premium_connection(
            ...
            quality_color = (
                premium_colors["process_green"]
                if avg_efficiency >= 90
                else premium_colors["warning_orange"]
            )
            ...
            outlet_main = FancyBboxPatch(
                (final_x, 9.0),
                5.0,
                3.5,  # Mismas dimensiones 4K
                boxstyle="round,pad=0.4",
            ...
            outlet_symbol = Circle(
                (outlet_center_x, 10.75),
                0.6,
                facecolor="white",
                edgecolor=premium_colors["primary_blue"],
                linewidth=3,
            ...
            ax.text(
                outlet_center_x,
                10.0,
                f"{quality_status}: {avg_efficiency:.1f}%",
                fontsize=18,
                fontweight="bold",
                ha="center",
                va="center",
            ...
            self._draw_premium_connection(
                ...
                outlet_left_x,
                outlet_center_y,
                {"type": "EFLUENTE_FINAL", "criticality": "high"},
                premium_colors,
            )
            ...
            ax.text(
                10,
                0.8,
                "Diagrama P&ID Generado por IA - H₂O Allegiant Professional Engineering",
                fontsize=11,
                ha="center",
                va="center",
                color=premium_colors["neutral_gray"],
                style="italic",
            )
            ax.text(
                10,
                0.4,
                f"Análisis Semántico: {len([e for e in classified_equipment if e.get('criticality') == 'high'])} Equipos Críticos | Complejidad del Sistema: {'Alta' if num_equipos > 6 else 'Media' if num_equipos > 3 else 'Básica'}",
                fontsize=10,
                ha="center",
                va="center",
                color=premium_colors["neutral_gray"],
            ...
            logger.info(
                f"📊 Análisis aplicado: {len(classified_equipment)} equipos con propiedades semánticas"
            )
            return base64_image
    ...
    def _get_premium_visual_properties(self, equipment: dict, colors: dict) -> dict:
        ...
        shape_mapping = {"complex": "hexagon", "moderate": "stadium", "simple": "round-rect"}

        # Símbolos industriales por stage
        symbol_mapping = {"primary": "⚡", "secondary": "🔄", "tertiary": "💧", "auxiliary": "⚙️"}
    ...
    def _draw_premium_equipment_node(
        self, ax, equipment: dict, x: float, y: float, color: str, shape: str, colors: dict
    ):
        ...
        if shape == "hexagon":
            ...
            hex_patch = Polygon(
                hex_points, facecolor=color, edgecolor="black", linewidth=border_width, alpha=0.9
            )
        ...
        ax.text(
            ...
            color="white",
            linespacing=1.3,
        )  # Texto principal con espaciado

    def _draw_premium_connection(
        self, ax, x1: float, y1: float, x2: float, y2: float, equipment: dict, colors: dict
    ):
        ...
        if length > 0:
            ...
            ax.plot(
                [x1_adj + 0.1, x2_adj + 0.1],
                [y1_adj - 0.1, y2_adj - 0.1],
                color="black",
    ...
    def _draw_premium_efficiency_panel(self, ax, efficiencies: dict, colors: dict):
        ...
        panel_shadow = Rectangle(
            (panel_x + 0.1, panel_y - 0.1),
            panel_width,
            panel_height,
            facecolor="gray",
            alpha=0.2,
    ...
    def _draw_premium_tech_panel(self, ax, equipment_list: list[dict], colors: dict):
        ...
        panel_bg = Rectangle(
            ...
            panel_height,
            facecolor="#f0f9ff",
            edgecolor=colors["tech_purple"],
            linewidth=4,
            alpha=0.95,
        )  # Consistente con panel eficiencias
        ax.add_patch(panel_bg)
    ...
    def _create_financial_chart_plotly(self, agent_data: dict[str, Any]) -> str:
        ...
        payback_years = agent_data.get("payback_years")
        roi_percent = agent_data.get("roi_percent")
        annual_savings = agent_data.get("annual_savings_usd")

        logger.info("💰 Datos financieros extraídos:")
        logger.info(f"  - CAPEX: ${capex:,.0f}")
        logger.info(f"  - OPEX anual: ${annual_opex:,.0f}")
        logger.info(
            f"  - Desglose CAPEX: {list(capex_breakdown.keys()) if capex_breakdown else 'No disponible'}"
        )
        logger.info(
            f"  - Desglose OPEX: {list(opex_breakdown.keys()) if opex_breakdown else 'No disponible'}"
        )
        logger.info(f"  - ROI: {roi_percent}%")
        logger.info(f"  - Payback: {payback_years} años")
        logger.info(
            f"  - Ahorros anuales: ${annual_savings:,.0f}"
            if annual_savings
            else "  - Ahorros anuales: No disponible"
        )
        ...
        try:
            ...
            if annual_savings and annual_savings > 0:
                ...
                colors_cash_flow = [
                    self.plotly_config.COLOR_SCHEMES["cash_flow"]["negative"]
                    if cf < 0
    ...
    def _generate_no_data_charts(self) -> dict[str, str]:
        ...
        return {
            ...
            "financial_executive": self._create_fallback_diagram(
                "No hay datos financieros del agente"
            ),
        }
    ...
    def _create_fallback_diagram(self, message: str) -> str:
        ...
        try:
            ...
            # Rectángulo de error
            error_box = Rectangle(
                (2, 2), 6, 2, facecolor="#fef2f2", edgecolor="#dc2626", linewidth=2
            )
            ...
            buf = BytesIO()
            plt.savefig(
                buf, format="png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none"
            )
            buf.seek(0)
    ...
    def generate_simple_process_diagram(self, agent_data: dict[str, Any]) -> str:
        ...
        try:
            ...
            main_equipment = agent_data.get("main_equipment", [])
            if not main_equipment:
                return self._create_fallback_diagram(
                    "No hay equipos disponibles para diagrama simple"
                )
