...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    def _normalize_role_superuser(
        ...
        *,
        ...
        if role is not None:
            data.is_superuser = role == UserRole.ADMIN.value
        ...
        if (
            ...
            data.role = UserRole.FIELD_AGENT.value
    ...
    async def validate_password(
        ...
        if not any(c.isdigit() for c in password):
            raise InvalidPasswordException(reason="Password must contain at least 1 number")
        ...
        if user and user.email:
            ...
            if len(email_prefix) >= 4 and email_prefix in password.lower():
                raise InvalidPasswordException(reason="Password cannot contain your email address")
    ...
    async def on_after_register(self, user: User, request: Request | None = None) -> None:
        ...
        await email_service.send_welcome(
            to_email=user.email, first_name=user.first_name or "there", login_url=login_url
        )
    ...
    async def on_after_forgot_password(
        ...
        await email_service.send_password_reset(to_email=user.email, reset_url=reset_url)
    ...
    async def on_after_request_verify(
        ...
        await email_service.send_verification(to_email=user.email, verify_url=verify_url)
