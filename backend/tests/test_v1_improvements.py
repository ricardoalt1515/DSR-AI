...
def test_subsector_extraction():
    ...
    subsector_kw, sector_kw, _contaminants = extract_user_keywords(MockCtx())
...
def test_weighted_scoring():
    ...
    print("  - Subsector 'food' match: +5")
    print("  - Contaminant 'organics' (BOD): +1")
    print("  - Total: 6")

    print(f"\nMunicipal Sewage score: {score_municipal}")
    print("  - No subsector match: 0")
    print("  - Contaminant 'organics' (BOD): +1")
    print("  - Total: 1")
...
def test_real_world_scenario():
    ...
    for rank, (score, case) in enumerate(scored, 1):
        ...
        if (
            "bod" in case["influent_characteristics"].lower()
            or "fog" in case["influent_characteristics"].lower()
        ):
