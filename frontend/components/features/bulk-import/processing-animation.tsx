...
const PHASES = [
    { key: "reading_file", label: "Reading file", description: "Extracting content from your document...", icon: FileSearch, progress: 20 },
    { key: "identifying_locations", label: "Identifying locations", description: "Finding locations and addresses...", icon: MapPin, progress: 45 },
    { key: "extracting_streams", label: "Extracting waste streams", description: "Identifying waste stream data...", icon: Bot, progress: 70 },
    { key: "categorizing", label: "Organizing data", description: "Classifying each item...", icon: Sparkles, progress: 90 },
];
...
export function ProcessingAnimation({
    ...
    const activePhase = currentPhaseIndex >= 0 ? PHASES[currentPhaseIndex] : PHASES[0];
        ...
        <div className="space-y-5">
            ...
            <div className="space-y-2">
                {PHASES.map((phase, index) => {
                    const isActive = phase.key === progressStep || (index === 0 && !progressStep);
                    ...
                    return (
                        ...
                        >
                            ...
                            {isComplete && <ChevronRight className="h-4 w-4 text-emerald-500" />}
                        </div>
                    );
                })}
            </div>
                        ...
                        <span className="text-muted-foreground">
                            {itemCount > 0 ? (
                                <>
                                    ...
                                    {filename && <> in <strong className="text-foreground">&ldquo;{filename}&rdquo;</strong></>}
                                </>
                            ) : (
                                <>Analyzing <strong className="text-foreground">&ldquo;{filename}&rdquo;</strong>...</>
                            )}
                        </span>
                    </div>
                </div>
