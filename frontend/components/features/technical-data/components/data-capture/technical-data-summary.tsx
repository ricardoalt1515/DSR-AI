...
export function TechnicalDataSummary({
	sections,
	className,
	onFocusSection,
	onFocusField,
}: TechnicalDataSummaryProps) {
	...
	const sectionStats = useMemo(
		() =>
			sections
				.filter(
					(section) =>
						section.fields.length > 0 && section.id !== "field-notes",
				)
				.map((section) => ({
					section,
	...
	const _highlightItems = HIGHLIGHT_FIELDS.map((item) => ({
		...item,
	...
	const _triggerFocusField = useCallback(
	...
	const tableRows = summaryRows.filter(
		(row) => row.sectionId !== "field-notes",
	);
		...
		<div className={cn("flex h-full flex-col gap-4", className)}>
			<Card className="surface-subtle rounded-3xl border-none shadow-none">
				<CardHeader className="gap-4">
					<div className="space-y-2">
						...
						<CardDescription>
							Capture status and derived metrics ready to share with the
							proposal agent.
						</CardDescription>
					</div>
					<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
						<div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
							...
							<Button
								...
								className={cn(
									"rounded-full px-3 text-xs",
									showSuggestedOnly ? "text-primary" : "text-muted-foreground",
								)}
						...
						<DropdownMenu>
							...
							<DropdownMenuContent align="end">
								<DropdownMenuItem
									onSelect={() => triggerFocusSection("project-context")}
									className="gap-2"
								>
					...
					<div className="grid gap-3 sm:grid-cols-2">
						{(Object.keys(SOURCE_META) as DataSource[]).map((source) => {
							...
							return (
								<div
									key={source}
									className="rounded-2xl bg-muted/30 p-4 text-sm"
								>
			...
			{!compactMode && (
				<Card className="flex-1 overflow-hidden rounded-3xl border-none bg-card/80">
					<CardHeader className="space-y-1">
						...
						<CardDescription>
							Each row consolidates the current value, unit and source to share
							with engineering or export to agents.
						</CardDescription>
					</CardHeader>
									...
									{tableRows.length === 0 ? (
										...
										tableRows
											...
											.map((row, index) => {
												...
												return (
													<TableRow
														key={`${row.sectionId}.${row.fieldId}`}
														className="align-top"
													>
														{showSection && (
															<TableCell
																rowSpan={rowSpan}
																className="bg-muted/30 font-medium text-foreground"
															>
																{row.sectionTitle}
															</TableCell>
														)}
														...
														<TableCell>
															<div className="inline-flex items-center gap-1 rounded-full bg-muted/60 px-2 py-1 text-xs font-medium text-muted-foreground">
																<meta.Icon
																	className={cn("h-3.5 w-3.5", meta.tone)}
																/>
			...
			<Card className="rounded-3xl border-none bg-card/80">
				<CardHeader className="space-y-1">
					...
					<CardDescription>
						Critical on-site observations from the team to contextualize the
						solution.
					</CardDescription>
				</CardHeader>
				<CardContent>
					...
					>
						{engineerNotes ? (
							...
							<span className="text-foreground">
								Record survey findings, risks, client agreements or relevant
								assumptions.
							</span>
						)}
					</button>
