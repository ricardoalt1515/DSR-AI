...
export interface ResourceInsight {
    ...
    lifecycle?: "Like-new" | "Good" | "Used" | "Degraded" | "End-of-life" | undefined;
    priceHint: string;
    insight: string;
    confidence: number;
}
...
const QUALITY_CONFIG = {
    High: { border: "border-green-500", text: "text-green-600 dark:text-green-400" },
    Medium: { border: "border-yellow-500", text: "text-yellow-600 dark:text-yellow-400" },
    Low: { border: "border-red-500", text: "text-red-600 dark:text-red-400" },
} as const;

// Empty State Component
function TopResourcesEmpty({ onUploadClick }: { onUploadClick?: (() => void) | undefined }) {
    return (
        <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center gap-4 py-12 text-center">
                ...
                <div className="space-y-1">
                    <h3 className="text-base font-semibold text-foreground">No photos uploaded</h3>
                    <p className="text-sm text-muted-foreground max-w-xs">
                        Upload site photos to get AI-powered material analysis and pricing insights.
                    </p>
                </div>
                {onUploadClick && (
...
function ResourceCard({ item, index, onImageClick }: {
    ...
    const lifecycleConfig = item.lifecycle ? LIFECYCLE_CONFIG[item.lifecycle] : null;
